...
### 人間が決めてほしいこと（レビュー時に判断）

1. **Enterキーでの即時検索**: デバウンス（300ms）のみとするか、Enterキーでデバウンスを無視して即発火させるか。本SPECではデバウンスのみとして進める前提だが、変更希望があれば指摘してください
2. **`buildIlikeValue`の共有化範囲**: 抽出先を新規`src/lib/search/like-pattern.ts`とし、`compatibilities/repository.ts`側もこの共有関数に差し替える（重複排除）。compatibilities側の回帰テスト範囲が広がるが、本SPECではこれを実施する前提。既存動作への影響が心配な場合は指摘してください
