...
- [ ] 製品名が空欄のまま「登録」するとバリデーションエラーが表示される
- [ ] JAN 重複・REF 重複のエラーは従来通り表示される
- [ ] メーカー名が未入力でも登録できる
- [ ] 製品名・メーカー名に日本語・記号（スラッシュ、スペース等）が入力できる

#### 一覧・表示

- [ ] 一覧テーブルに「製品名」「メーカー名」列が表示される
- [ ] メーカー名が未設定の製品は「—」（または空欄）で表示される
- [ ] 既存レコード（移行前データ）が一覧で正常に表示される

#### 非機能

- [ ] マイグレーション適用後、既存データが壊れていない（DEFAULT '' または NULL 許可で対処）
- [ ] 管理者・施設スタッフ双方が登録・編集できる（RLS は既存ポリシーのまま）
