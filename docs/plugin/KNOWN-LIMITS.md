...
- 各検査が「何を見るか」は `scripts/lib/check-scopes.json`（生成物）に入っている:
  ...
  `both` = 自己検証と実態の走査が同居
- **入口の報告は 4 値**（2026-09-12）: `見た` / `対象なし` / `落ちた` / `実体なし`。
  以前は「合格 N」しか出さず、**対象が無いので何もしなかった**ものを合格に混ぜていた。
  実測では、導入先で回る 29 本のうち実際に相手を見たのは 13〜14 本で、残りは
  「持っていないので黙った」——なのに表示は「合格 29」だった。
  `対象なし` の一覧は毎回出るので、**何が守られていないか**が読める
- 判定の仕方: 検査が出力に「対象なし」を含めたとき、**空の木でもう一度回して出力を比べる**。
  変わらなければ本当に何も見ていない。変われば、この導入先の何かを見ている
  （印は場面ごとに出るのに分類は検査ごとなので、文言だけで数えると守りを過小に見せる）
- **限界**: 出力に実行ごとに変わる実測値を含む検査は、差分が必ず出るので `見た` に倒れる
  （空の木での実測: 29 本中 1〜2 本）。また **「環境不足 → 確認不能」はまだ分けていない**。
  jq / node が無くて降りた場合は `落ちた` か `対象なし` に混ざる
- **門**（`scripts/check-plugin-check-coverage.test.sh` の scenario 5 / 5b、2026-09-12）:
  配る検査を**空の git リポジトリ**で 1 本ずつ回し、落ちるもの・「対象なし」と言わないものを落とす。
  この製品固有の前提（supabase の migration・統合テストの記録）が要る検査は
  `plugin-layout.json` の `emptyRepoExempt` へ**理由つき**で宣言する。
  免除は腐るので、5b が「空の木で通るようになったのに載ったまま」の行を落とす
  （門を入れた初回の実行で、書いた本人の宣言 2 件がこれで落ちた）
