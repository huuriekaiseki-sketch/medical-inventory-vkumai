...
| I-035 | 消耗品発注の明細は、**自施設の**、**使用停止でない**消耗品だけを指す（2026-09-09 の実測で、他施設の消耗品も使用停止の消耗品も指せることが分かった。**混乱した代理人**: 呼び出し元は正しく認可されていて、渡された参照先だけが他人のもの）。**過去の明細は使用停止にしても残る**（履歴は履歴のまま） | 共有関数 `assert_facility_owns(..., 'orderable_consumable', ...)`（20260909080000）を `create_consumable_order_atomic` が呼ぶ（check_violation）。当初は RPC 内に直接書いたが（20260909060000）、同じ書き忘れが 2 件出たので共有部品にした | 他施設の消耗品 ID・使用停止した消耗品 ID・存在しない ID で発注 RPC を呼ぶ | 23514（`is not orderable`）。自施設の生きた消耗品なら通る（対照）。明細が 1 つでも悪ければ発注ごと残らない | `supabase/__tests__/integration/consumable-order-items-boundary.integration.test.ts`、`src/lib/consumable-orders/__tests__/repository.test.ts`（利用者向けの文言への写し）。効き目は `scripts/lib/rls-mutants.json` の RM-016（呼び忘れ）と RM-018（共有関数の空振り）が実測する | 実装済み |
| I-036 | 返却の紐付け先は、**header の発注 ID も明細の発注明細 ID も**自施設のものだけ（2026-09-09 の実測で、**明細を 1 つも紐付けなければ header の発注 ID が素通りする**ことが分かった。明細側の検証は 20260908040000 で入っていたが、明細が 0 件だと WHERE に 1 行も入らない） | 共有関数 `assert_facility_owns(..., 'loan_order' / 'loan_order_item', ...)`（20260909080000）を `create_loan_return_atomic` が 2 回呼ぶ（どちらも foreign_key_violation）。当初は RPC 内に直接書いた（header は 20260909070000、明細は 20260908040000） | 他施設の発注 ID を header に入れ、明細を紐付けずに返却 RPC を呼ぶ | 23503。自施設の発注なら通る（対照）。明細に他施設の ID を入れた場合も 23503 | `supabase/__tests__/integration/rpc-reference-boundary.integration.test.ts`。効き目は `scripts/lib/rls-mutants.json` の RM-017（呼び忘れ）と RM-018（共有関数の空振り）が実測する | 実装済み |
| I-037 | 代理店商品を削除すると、その仕切値の変更履歴が残らない（孤児にならない）。**逆に、履歴があることを理由に削除が止まることも無い**（2026-09-10 まで止まっていた。契約 O-052 は「admin + aal2 が消せる」だが、`price_histories` からの FK が `ON DELETE` 指定なしで入っており、仕切値を一度でも変えた商品は**誰にも消せなかった**。履歴は GRANT が SELECT のみなので先に消す回避もできない） | トリガー `distributor_products_delete_price_histories`（BEFORE DELETE、20260910000000）。実 FK の列と緩い参照（`entity_type`/`entity_id`）の**両方**を消す。院内価格側の同じ規則は `hospital_prices_delete_price_histories`（20260906000007） | 仕切値を変えて履歴を作った代理店商品を service_role で DELETE する | 削除が通り、その商品の履歴が 0 件。**別の商品の履歴は残る**（消しすぎない対照） | `supabase/__tests__/integration/business-invariants.integration.test.ts`、`supabase/migrations/__tests__/delete_price_histories_with_distributor_product.test.ts` | 実装済み |
