...
| 契約（H-03） | 決めたことと動くものが食い違わない（操作の契約・層の突合・入口の検証） | **機械**（npm test と hooks-test が毎回回す） | `npm test`<br>`bash scripts/check-operation-contracts.test.sh` | 44 本 |
...
（ハーネス 8 件・検査 159 本・台帳 8 件。うち `scripts/**/*.test.sh` の 157 本は**この表で全数**——どこにも属さない検査があれば生成そのものが落ちる。残り 2 本は vitest 側から**手で足したもの**で、書き忘れは検知されない（限界の節））
