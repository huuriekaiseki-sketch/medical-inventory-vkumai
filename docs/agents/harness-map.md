...
| 監視・観測（H-07） | 起きたことに気づける（夜間検査・鮮度・記録漏れ） | **機械**（夜間検査は pg_cron、鮮度は SessionStart / Stop hook。**本番の監視は外部待ち**（#757-8）） | `scripts/check-integration-freshness.sh`<br>`scripts/check-e2e-freshness.sh`<br>`scripts/maintenance-digest.sh` | 36 本 |
...
| `check-mutants.json`#minMutants | 判定エンジンの壊し方（下限）（H-06） | 件 | **34** |
| `rls-mutants.json`#mutants.length | RLS・RPC の壊し方（H-06） | 件 | **18** |

（ハーネス 8 件・検査 123 本・台帳 6 件。**検査はこの表で全数**——どこにも属さない検査があれば生成そのものが落ちる）
