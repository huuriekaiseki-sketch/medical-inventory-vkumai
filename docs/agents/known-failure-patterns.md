...
**再発（同日、配布形態の変化・プラグイン v1 の実走ドリル）:** 生成物をクリーンな検証リポジトリに
`--plugin-dir` で読み込んで 1 ターン起動したところ、hook 15 本が `cd "$(dirname "$0")/.."` で
「リポジトリルート」へ移動する前提が崩れていた。プラグインではスクリプト位置がプラグインルートに
なり、Stop hook の状態ファイルと `resolve_log_dir` の解決先が導入先でなくなる（月次サマリが中心
リポジトリの `logs/` を読んでいた）。リポジトリ内のテストは全件 GREEN のままだった。加えて生成した
manifest の `hooks: "./hooks/hooks.json"` が自動読み込みと重複し「Hook load failed」になっていた
（hook 自体は動くため気づきにくい）。修正は `cd "${CLAUDE_PROJECT_DIR:-$(dirname "$0")/..}"`
（hook は CLAUDE_PROJECT_DIR を受け取る）と manifest から `hooks` を外すこと。
**「入力形式」「運用手順」に続く 3 つ目の無音死の型は「配布形態の変化」**（スクリプトの置き場所と
実行時の cwd が分かれる）。hook を書くときは「自分の位置」と「作業対象のリポジトリ」を区別し、
後者は hook 入力の `cwd` / `CLAUDE_PROJECT_DIR` から取る。
