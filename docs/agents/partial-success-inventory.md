...
| M-021 | 招待: 回数枠の消費 → `auth.admin.inviteUserByEmail`（利用者の作成 → メール送信） | **利用者側は残らない**（2026-09-07 に SMTP を止めて 3/3 実測: 500 `Error sending invite email`、`auth.users` 0 件・`auth.identities` 0 件・`one_time_tokens` 0 件・GoTrue の監査行も無し。GoTrue が利用者行ごとロールバックする）。**残るのは消費済みの回数枠**（Q-020、送信の前に数える） | 「招待メールの送信に失敗しました」。枠が減ったことは見えない | 押し直せば新規の招待として通る（利用者行が無いので M-020 の再送ですらない）。ただし押すたびに 1 日 50 通の枠が減り、**メールは 1 通も出ない** | `src/app/api/admin/users/__tests__/route.test.ts` | 中間状態あり（#757-38: 送信に失敗した分の枠を戻すかを決める） |
...
- **中間状態あり**は 2 行。M-012（マスタ作成の再送）は admin 専用で件数が少ないので運用で許容し、`clientRequestId` の拡張は引き金が来たら。M-021（招待の回数枠）は**残るのが行ではなく枠**という形で、戻すかどうかは人の判断が要る。
