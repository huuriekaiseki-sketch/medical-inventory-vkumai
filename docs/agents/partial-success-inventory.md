...
- 「守るテスト」列はバッククォートでパスを書く。無い行は `未`。`scripts/check-partial-success-inventory.test.sh`
...
| M-021 | 招待: 利用者は作られたがメール送信だけ失敗（SMTP 停止・送信上限） | GoTrue は送信失敗をエラーで返すが、利用者行が残るかは未実測 | 「招待メールの送信に失敗しました」 | M-020 のとおり再招待は安全（残っていても同じ利用者に再送、残っていなければ新規） | 未 | 未確認（#757-38: ローカルの SMTP を止めて実測し、利用者行の有無を M-020 のテストに足す） |
...
- **未確認**の M-021 は SMTP を止めた実測、M-041 は機能追加時。

## 限界

- **実際に途中で止めて測ったのは一部。** 多くは経路を読んだ判断で、
  「原子的」と書いてあっても、実際に明細で失敗させてヘッダが残らないことを確かめたのは
  発注・返却の 4 経路だけ。**SMTP を止めた実測（M-021）はまだ**。
...
- **外部サービスの中の段は見えない。** GoTrue の `inviteUserByEmail` は
  「利用者の作成 → メール送信」を内部で行うが、その境目で止まったときに何が残るかは
  こちらからは制御も観測もできない（M-020 / M-021）。
