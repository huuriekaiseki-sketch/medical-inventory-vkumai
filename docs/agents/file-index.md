...
- **この表の網羅性は誰も検査していない。** 新しいスクリプトを足しても、ここに書かなければ載らない。
  ...
  「載せ忘れ」は検知できない。
  **2026-09-13 実測: 検査 149 本のうち、この表に名前が出るのは 29 本（19%）。載っていないのが 120 本。**
  つまり**載っているほうが例外**で、しかも 29 本はここ数日に作ったものへ強く偏っている
  （手本として何度も参照される `check-export-paths.test.sh` すら載っていない）。
  **「ここに無い＝まだ無い」と読んではいけない。** 既にあるかを確かめる用途には
  `ls scripts/` か `grep` を使い、この表は**載っている分の設計意図と限界を読む**ために使う。
  網羅を機械化しない判断は据え置く（1 行の説明に意味を持たせているので、
  機械生成にすると「名前だけ 149 行」になって読む価値が消える）。
...
| `scripts/lib/scan-suspense-gaps.mjs` / `scripts/check-suspense-gaps.test.sh` | **`useSearchParams()` が `<Suspense>` の内側にあるか**（2026-09-10）。**「next build が落とすはず」を壊して確かめたら落ちなかった**——公式の防御は静的ページ限定で、このアプリは全ルートが動的レンダリングのため**適用されない**（実測）。同じ欠陥を Sweep（LLM）も外している。**LLM もフレームワークも lint も捕まえていなかった。**同じファイルの `<Suspense>` か、**import 元が Suspense の内側で使っている**ことを見る（別ファイルで包む正しい形を誤検知しないため）。逃がすときは `// suspense-exempt: 理由`。限界: JSX を構文解析せず入れ子は最初の閉じで切る／import は 1 段だけ／`next/dynamic` は追えない。変異 CM-052 / CM-053 が反証を持つ |
| `scripts/lib/scan-definer-authz-gaps.mjs` / `scripts/check-definer-authz-gaps.test.sh` | **client から呼べる `SECURITY DEFINER` に、呼び出し元を確かめる判定があるか**（2026-09-10）。`SECURITY DEFINER` は RLS を通らないので、関数の中で確かめないと境界が無い。既存の `scan-guard-regressions.mjs` は「**前の版にあった判定が消えた**」しか見ず、**最初から判定の無い新しい関数は素通り**していた。`user_facilities` を引いているだけは認可に数えない——「**誰かが**所属しているか」は「**呼び出し元が**所属しているか」ではない。実測でできた穴: 仕込んだ同型の欠陥を Sweep（haiku）は **5 回とも見逃し**、sonnet は見つけたが 1 回 $2.50 だった。**「必ず全件見る」を守らせるより機械で数えるほうが確実で安い。**検査は eval の fixture をそのまま入力に使い、検査と eval が同じ欠陥を見ていることを固定する。逃がすときは migration に `-- definer-open: 理由`（理由が要る）。限界: 静的解析で、判定が効く位置にあるかは見ない（それは変異検査）。`GRANT ... ON ALL FUNCTIONS IN SCHEMA` は追えない。変異 CM-046〜CM-048 が反証を持つ |
...
| `e2e/api-route-registry.ts` / `src/__tests__/api-attack-matrix-ratchet.test.ts` | **実在する API route が攻撃表（P-017）に載っているか**を答える 1 か所（2026-09-10、E-068）。`src/app` を歩いて Route Handler を列挙し、`e2e/api-attack-matrix.ts` と**両方向**で突き合わせる（表に無い route ／ 実在しない route の両方で落ちる）。**元は攻撃 spec の中にあり、実質動いていなかった**——E2E は節目実行のうえ、同じ describe の `test.skip`（フィクスチャ・`SUPABASE_SERVICE_ROLE_KEY`）に巻き込まれて Supabase を止めている間ずっとスキップされていた。ファイルを読むだけの検査なので `npm test` へ移した。列挙は `route.tsx` や `export const GET` / `export ( h as GET )` も拾う（実測では全 34 本が `route.ts` + `export async function` だが、**0 件のうちに抜け道を無くす**）。限界: route の**存在**を見るだけで、認可が正しいかは見ない（それは総当たり攻撃 spec の担当） |
| `scripts/lib/scan-skip-scope.mjs` / `scripts/check-skip-scope.test.sh` | **前提（スキップ条件）の範囲が、それを必要とする検査より広くないか**（2026-09-10、C-033）。`test.describe` 直下の条件つき `test.skip` は describe 内の全テストに効くので、**DB もブラウザも要らない検査まで一緒に黙る**。しかもスキップは失敗ではないのでレポート上は緑と同じ色。同期テスト（`await` できない＝前提に到達しようがない）だけを対象にして過検知を避ける。逃がすときは `// skip-scope-ok: 理由`。実測: spec 16 本 / describe 24 / 条件つき skip 27 に対し違反 0 件。**修正前の spec を入力にするとちょうど 1 件**を名指しする（対照）。限界: 静的解析で describe の範囲はインデントで取る／`async` のテストは見ない／実行時のスキップ件数は見ない |
...
| `scripts/show-harness-evidence.sh` | **役割ごとに「いま測れているか」を実測の記録から出す**（2026-09-10、レビューの設計提案 4）。地図の状態欄は手書きの「あり / 一部」で、登録簿自身が「『あり』は中身の十分性を保証しない」と書いていた。**測定 / 合格 / 最新 を潰さずに別々に**出す——一度も回していない・赤のまま・木が変わっている、は別の話。記録は機械ローカル（`logs/` は git 管理外）なので**コミットする文書へは焼き込まない**（焼き込むと環境ごとに生成物が割れ、C-010 を作り直すことになる）。**木で表せない結果はコミットで見る**（`watchCommit`、2026-09-10）——マージ予行（H-08）は「いま main に何が入っているか」に依存し、どれか 1 つのディレクトリの木では表せない。無理に木へ載せると「変わっていないのに最新」と言う（C-010）。**watch も watchCommit も無い証拠は登録できない**（最新かを永久に判定できないため）。限界: HEAD の木のハッシュだけを見る（未コミットの書き換えは Stop hook の担当） |
| `scripts/rehearse-merge.sh` / `scripts/lib/rehearse-merge.mjs` / `scripts/lib/merge-queue.json` | **この順で main へ入れたら衝突するかを、作業ツリーを触らずに調べる**（H-08）。`git merge-tree` と `commit-tree` だけを使う。**起点が遅れていたら合否を出さずに止まる**（終了コード 3。2026-09-10 に追加）——既定の起点 `origin/main` は GitHub 停止で 9/6 から凍っており、実際に積む先の main は 246 コミット先だった。その起点で回すと**実在しない衝突 10 件**が出る（実際の main を起点にすると 37 本すべて「済み」で 0 件）。「衝突している」でも「していない」でもなく**誰も聞いていない問いへの答え**なので合否に混ぜない（C-025）。順番が空のときも落とす（0 本を合格と読まない。C-021）。変異 CM-042 / CM-043 が反証を持つ |
...
| [`docs/agents/sweep-recall-baseline.md`](./sweep-recall-baseline.md) | **Sweep 4 層の見逃し・誤警報の基準値**（2026-09-10）。条件（プロンプト・エージェント定義・fixture・モデル・採点器）を記録した状態で測った最初の回。**誤警報は 4 層とも 0、問題は一貫して見逃し側**。`sweep-db` と `sweep-types` は仕込んだ欠陥を外し、`sweep-ui` は**囮のあとの本物**を外した。手順を厚くしても再現率は動かず、同じ手順を sonnet は実行できて当てた（$2.50）。→ **Sweep を強くするより、外しても困らない形（機械の検査）へ寄せる**という判断の根拠 |
| `scripts/lib/judge-sweep-recall.py` / `scripts/lib/judge-sweep-recall.test.sh` | **Sweep の recall を採点する決定的ロジック**と、その回帰テスト（2026-09-10、R11）。直す前は「期待パスの部分文字列 AND 期待キーワード」だけを見ており、**「確認しましたが問題はありません」という見逃しの回答も HIT** にしていた。出力契約の `FINDINGS: <件数>`（無ければ「指摘なし」の語）で**指摘の有無**を先に判定し、`expectNoFinding` の**陰性対照 fixture** は逆向きに採点する  **`--count` で指摘の件数も返す**（2026-09-10）。陰性対照は「その fixture への指摘」しか過検出に数えないので、**実在ファイルへの誤指摘が 0 件として素通り**していた。Sweep は毎回リポジトリ全体を掃くので、件数を `findingsReported` として記録し PM-050 で出す。**本物か誤りかは分けない**（分けるのは人の仕事で、測れるのは増減だけ）。件数が読めない回は `-1` で返し、0 件と混ぜない。**さらに 2026-09-10、判定を「同じ指摘の中でそろっているか」へ変えた**（R11 の後段）——それまでは本文全体で「パスがある AND キーワードがある」を見ていたので、**別々の指摘に分散していても HIT** だった（Sweep は全体を掃くので無関係な指摘が並ぶのが普通）。指摘の区切りは出力契約が定めていないので、**行頭の箇条書き・番号・見出し**で切り、**インデントされた行は切らない**（1 件が「ファイル: …／問題: …」と入れ子で続く形を分断しないため）。`--loose` で従来の判定も出せる——`eval-sweep-recall.sh` が両方を記録し（`loosePass`）、**切り替えの影響を後から測れる** |
| `scripts/check-eval-fixtures-neutral.test.sh` | **eval の fixture が「これは評価用だ」と名乗っていないか**（issue #731 → 2026-09-10 に E-070 で拡張）。最初はコード中の自己申告コメント（「ベンチマーク用 fixture」「意図的に再現」）だけを禁じ、**ファイル名・識別子は「import パスとして必ず現れるから」と除外していた**。その除外が実測で否定された——回答本文を残した 1 回目で、Sweep が「`eval_fixture_recall_items` テーブルは eval フィクスチャ専用で本番対象外」と書いて指摘から外していた。**名前だけで伝わっていた。**いまは名前も対象で、fixture 側は業務らしい名前（`shift_handover_items` / `ward_supply_items` / `sterilization_log_items` / `facility_shift_notes`）に改名済み。限界: 禁止語の一覧に載っている語しか見ない（「これはテスト用」と別の言い方をされたら通る） |
| `scripts/lib/scan-exemptions.mjs` / `scripts/check-exemptions.test.sh` / `scripts/lib/exemption-budget.json` | **検査の「逃がし口」を数える**（2026-09-10）。`// suspense-exempt:` `// skip-scope-ok:` `-- definer-open:` `-- drops-guard:` `@ts-expect-error` `eslint-disable` の 6 種を、プロダクトコード（src / supabase / e2e）で数えて上限と突き合わせる。どれも**理由を書かせている**ので 1 件ずつは納得できるが、**何個あるかを誰も数えていなかった**——増えれば検査は形骸化する。実測すると自作の印はすべて **0 件**だったので 0 で張った（`eslint-disable` だけ 14 件）。理由の有無も見る（実測で 9 件が理由なし。うち 1 件は走査側の過検知で、直前の WHY コメントを理由と認めるよう直した）。限界: **検査自身の定義・テスト・ルールブックは数えない**（印の文字列がそこには必ず現れるため）。印を使わずにすり抜ける書き方は分からない。変異 CM-064 / CM-065 が反証を持つ |
| `scripts/lib/scan-holdout-isolation.mjs` / `scripts/check-holdout-isolation.test.sh` | **held-out（評価専用の未公開セット）が本当に隔離されているか**（2026-09-10）。`sweep-db-holdout` の expected.json には `heldOut: true` と書いてあったが、**この印を読むコードが 1 つも無かった**（C-010: 印だけがある状態）。見るのは 3 つ——`*-holdout` の全 case に印があるか／普通のセットに印が紛れていないか／held-out の**名前**（セット名・case 名・fixture のファイル名）が `.claude/` 配下に 1 つも出てこないか。**held-out セットが消えたら落ちる**（この検査が何も守らなくなる状態を許さない。本当に持たない導入先は `HOLDOUT_SCAN_ALLOW_ZERO=1`）。見張る名前は「**held-out にしか無い名前**」＝普通のセットにも現れる名前を引いた差集合（最初は `/holdout|eval[-_]fixture/` を含む名前で絞っていたが、E-070 で fixture を業務らしい名前へ改名した瞬間に**何も見張らなくなった**——印で絞る判定は印を外すと空振りする。C-011）。限界: **参照が無いことは「使っていない」ことの必要条件でしかない**——人が fixture を読んでプロンプトを書き換えたら、名前が出なくても過学習は起きる。変異 CM-062 / CM-063 / CM-066 が反証を持つ |
...
| `scripts/check-actuator-inventory-coverage.test.sh` | **是正の棚卸しと hook の実登録を突き合わせる**（2026-09-12）。`.claude/settings.json` と `.codex/hooks.json` に登録されて動いているのに [`actuator-inventory.md`](./actuator-inventory.md) に 1 行も無い検知 hook を落とす。**書いた直後に、人が手で数えて見落としていた 8 本目**（`codex-ai-check-suggest.sh`）を見つけた。記録専用 hook は名前で除外せず `systemMessage` を出すかで実測して分ける（名前を焼き込むと新しい記録専用 hook が増えたときに腐る）。限界: **表のどこかに名前が出れば「載っている」と数える**ので、分類（block / ask / warning-only）が正しいかは見ない——2026-08-10 の 1 例目（`verify-claims.sh` を warning-only と誤記）はこの検査では捕まらない |
| `scripts/check-support-script-coverage.test.sh` / `plugin-layout.json` の `supportScriptsNotDistributed` / `supportScriptsUnclassified` | **支援スクリプトの「配る・配らない」を書く場所と、その突合**（2026-09-12）。検査（`*.test.sh`）と agent / skill / workflow には層の表との突合があったのに、`scripts/` の非テストスクリプトだけ無く、**`checksNotDistributed` に当たる「配らないと決めた」表そのものが存在しなかった**ので、表に無いものが判断の結果か足し忘れか区別できなかった。配布方針の決定は製品の判断なので**未分類の置き場と上限**（`unclassifiedMax`。増やせないが減らせる）を先に作ってある。層の表は**全体を歩いて `.sh` のキーを拾う**——キーを手で選ぶと母集団を外す（実際 4 キーで数えて 33 本という誤答を出した。正しくは 25 本） |
| `scripts/check-aidd-config-schema.test.sh` / `scripts/lib/aidd-config.schema.json` / `scripts/lib/validate-aidd-config.mjs` | **導入先の設定（`aidd.config.json`）がスキーマに従うか**（2026-09-12）。スキーマは前からあったが**検証する口が 1 つも無く、誰も読んでいなかった**。依存を増やさないため検証器は自前で、**知らない語彙が出たら黙って通さず落ちる**（対応していない制約を「無かったこと」にしない）。設定が無い導入先では「対象なし」と言って降りる |
