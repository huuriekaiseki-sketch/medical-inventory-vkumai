...
## なぜ P-017 の攻撃 spec を Playwright の別プロジェクトへ隔離し、しかも「先頭」に置いたか（2026-09-07）

**背景:** `e2e/api-cross-facility-attack.spec.ts` の「施設 A のデータは漏れず・変わらない」は、
攻撃の前後で施設 A の全行を service role でスナップショットし、文字列比較する。この判定は
「攻撃の間、施設 A に誰も書き込まない」という前提の上に立っている。ところが施設 A は
cross-facility フィクスチャの共有資源で、`e2e/consumable-orders.spec.ts` も消耗品を作る。
Playwright は既定でファイル単位に並列実行するため、`--workers=5` では他テストが作った行が
「攻撃でデータが変わった」として `+ Array [ "consumables" ]` で落ちた（2026-09-07 に実測再現）。
`--workers=1` で緑になっていたのは直列化のおかげではなく、ファイル名の辞書順で攻撃 spec が
たまたま最初に走っていたからで、たまたま守られていた前提だった。

**なぜスナップショットの比較対象を絞る案を退けたか:** 「攻撃前から存在した行だけを比較する」に
すると、他テストの INSERT は無視できるが、攻撃自身が作った行も同時に見えなくなる。
「他テストの INSERT」と「攻撃の INSERT」を行の内容から区別する確実な手段が無い
（作成者を記録する列は全テーブルには無い）ため、誤検知を消すぶんだけ P-017 の ratchet が
「作られた行を見逃す」側へ弱くなる。弱くしない条件を満たせないので採らない。

**なぜ攻撃専用の施設を新設する案も退けたか:** 「他テストが触らない施設」を用意しても、
E2E の既定ユーザーは `is_admin()` が真の admin で、施設一覧には全施設が出る。
`consumable-orders.spec.ts` は「一覧の最初の施設」を選んで書き込むため、施設を増やしても
「最初の施設」に選ばれうる。規約（この施設には書かない）でしか守れず、機械的な保証にならない。

**なぜ先頭に置くか（後ろではなく）:** Playwright は依存プロジェクトが failed だと依存元の
テストを skip する。攻撃 spec を最後に置く（他プロジェクトに依存させる）と、無関係な E2E が
1 件落ちた日に P-017 が黙って実行されない。セキュリティの ratchet が静かに走らなくなる方が、
攻撃 spec が落ちた日に残りの E2E が skip される方より危険なので、攻撃 spec を先頭に置いて
「必ず走る」側に倒す。残りの E2E だけを見たいときは `--no-deps` で依存をたどらずに走らせる。

**How to apply:** e2e に spec を足すときは何もしなくてよい（自動的に後段の並列プロジェクトに入る）。
攻撃 spec と同じく「他テストが書き込まない前提」を持つ spec を足すときは
`e2e/project-isolation.ts` の隔離プロジェクト側へ入れる。隔離の形（分離・依存の向き・重複なし）は
`src/__tests__/e2e-project-isolation.test.ts` が `npm test` で固定する。

## なぜ検査（*.test.sh）を「対象スクリプトと同じ層に自動で付いていく」形で配ることにしたか（2026-09-07）

**背景:** プラグイン v1 は hook 本体・エージェント・スキル・Workflow だけを配っており、検査を 1 本も
配っていなかった。派生先には「止める仕組み」が渡るが、「その仕組みが壊れていないことを確かめる手段」は
渡らない。実際、#757 の 19 で見つけた外部キーの索引漏れは、検査を書いて初めて 1 件追加で見つかった
（`distributor_products.product_id`）。ルールだけ配って検査を配らないと、派生先では同じ発見ができない。

**なぜ層の表に全部を書かないか:** 検査は今 67 本あり、その大半は「ある hook を守る検査」で、層は対象と
同じに決まっている。二重に書くと必ず食い違う（片方だけ直す事故が起きる）。そこで
`check-x.sh` に対する `check-x.test.sh` は自動で対象と同じプラグインへ付いていく形にし、
層の表には「対象を持たない構造テスト」と「自動判定の上書き」だけを書く。

**なぜ配らない検査を消さずに理由つきで残すか:** 「配らない」は判断であって、忘れではない。
理由を書かせると、後から「この理由はもう成立しない（設定に出せば配れる）」と気づける。
理由が空の行と、ファイルが消えたのに残っている行は
`scripts/check-plugin-check-coverage.test.sh` が落とす。

**なぜ検査の同梱閉包を緩めたか:** 検査は fixture として存在しないファイル名（`scripts/check-a.sh` のような
架空の名前）を書く。これを実行時参照と見なすと例外リストが際限なく増える。検査については
「対象スクリプトが同じプラグインに同梱されているか」だけを見る形にした。本当に困るのはそこだけで、
実際この変更で「対象は共通側にあるのに検査だけアダプター側へ回そうとした」6 本を機械が止めた。
