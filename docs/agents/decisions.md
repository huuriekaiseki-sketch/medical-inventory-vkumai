...
深掘り調査へ誤って振り分けられた）を受けて設計を見直した。

**判断1: メタ改修判定を既存のキーワードマッチ・パスマッチより「先に」評価する。**
issueの検討事項にあったとおり、否定文脈の解析（"触れない"を構文的に検出する）は今回の
スコープにしなかった。代わりに、changedFilesという「実際に何を変更したか」というより
確実な情報源を先に見ることで、taskDescriptionの表現に依存せず誤検知を回避する設計にした。
これは既存のTRI/RISK基準の思想（`docs/agents/common.md`「判定優先順位: changedFilesの
パス一致を優先し、taskDescriptionのキーワード一致は補助判定」issue #286）と整合する
延長線上の判断であり、新しい思想を持ち込んでいない。
