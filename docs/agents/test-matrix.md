...
| フレーキー検知 | ✅ | 節目 | 週次 cron（日曜 20:00 UTC）。揺れやすい統合テスト（concurrency / idempotency）や検知の仕組みに触れた PR は手元で回す | 同じテストを N 回回し、通ったり落ちたりするテスト（揺れ）と毎回落ちるテスト（バグ）を分けて出す。揺れは「たまたま通った緑」を生み、Free プランでは緑の信頼性が防御線そのものになる（issue #757 の 14）。見つかったら `[flaky] <suite>` の issue を作る／追記する。毎 PR で回すと Actions 無料枠が枯渇するため週次 | `scripts/check-flaky-tests.sh`、`scripts/lib/flaky-aggregate.mjs`、`scripts/lib/flaky-issue.sh`、`scripts/check-flaky-tests.test.sh`、`.github/workflows/flaky-detection.yml` | flaky | Google Flaky Tests / Test Reliability | `bash scripts/check-flaky-tests.sh --runs 5`、`bash scripts/check-flaky-tests.sh --runs 3 --config vitest.integration.config.ts` |
