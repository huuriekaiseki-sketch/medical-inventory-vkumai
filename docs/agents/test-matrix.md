...
| 業務不変条件（DB 制約） | 🟡 | 変更時 | migration・発注/返却/価格のリポジトリに触れた PR | 「数量は 1 以上」「金額は 0 以上」「状態は戻らない」等を DB の CHECK / トリガーが守り、破る操作が拒否されることを実 DB で見る。正本は `docs/agents/invariant-catalog.md`（I-xxx、カタログとテストの整合は構造テスト）。集計をまたぐ条件（I-05x）は夜間検査が未整備 | `docs/agents/invariant-catalog.md`、`scripts/check-invariant-catalog.test.sh`、`supabase/__tests__/integration/business-invariants.integration.test.ts` | invariants | Google Medium / DB constraint testing | `npm run test:integration` / `bash scripts/check-invariant-catalog.test.sh` |
