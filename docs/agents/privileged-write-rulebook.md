...
| W-023 | `src/lib/security/hidden-row-denial.ts` | 書かない。RLS で 0 件だった ID 指定の 1 件取得（facilities / hospital_prices）について、行が実在するかを service_role で 1 回だけ読み、あれば `access_denials` に拒否（W-020 経由）を残す | 無し | 無し（route が 404 を返す直前に呼ぶ。読むのは facility_id だけで、結果は呼び出し元に返さない） | 存在の有無が応答に漏れないことは「404 の文言が本当に無いときと同じ」で守る。記録できるのは actor_id が本人（`requireAuth` 済み）の拒否だけ。aal1 の利用者は proxy の MFA ガードが /api に届く前に止めるので、隠れる理由は所属なし・非 admin に限られる。**一覧の空配列と PostgREST 直叩きは対象外**（#757-24 の残り） | `src/lib/security/__tests__/hidden-row-denial.test.ts`、`e2e/admin-audit.spec.ts`（施設 B が施設 A の ID を指定 → 404 のまま記録 +1、存在しない ID は増えない） | 実装済み |
