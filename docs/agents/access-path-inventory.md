...
| X-022 | 管理画面（`/admin/*`、`/api/admin/*`） | admin | 利用者 JWT（admin）+ サーバー側の service_role key | 利用者の招待・削除、所属とロールの変更、レポート | proxy の admin ガード + `requireAdmin`。**proxy の admin 拒否は `access_denials` に残る**（`/login` の Server Component が印を読んで記録。#757-24 で実装済み） | P-017（admin 系 route）、P-021、P-060（`user_facilities`）、P-063。`auth.users` の変更（招待・削除）は監査ログの対象外（auth スキーマ）だが、2026-09-07 から `privileged_operations` に成功・失敗を記録（W-022。2026-09-13 に「未」の表記を訂正） | 一部 |
