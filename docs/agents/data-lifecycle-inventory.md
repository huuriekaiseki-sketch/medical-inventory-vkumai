...
- 「守るテスト」列はバッククォートでパスを書く。無い行は `未`。`scripts/lib/check-catalog.mjs`（登録簿の `data-lifecycle`）
...
| D-032 | GitHub / GitLab のリポジトリ（**履歴を含む**） | 業務データは無い。鍵・トークン・秘密鍵は**一度も入っていない**（2026-09-08 に到達不能分を含む blob 4,370 件・49.6 MB を走査してヒット 0）。ただし 7d10a9e（2026-06-18）の `supabase/.temp/` に**本番の project ref・organization id・pooler の接続文字列**（パスワードは無し）が残る | **消えない**。消すには履歴の書き換えが要り、全 clone が壊れる。公開の判断（#660）と同時に決める | `bash scripts/check-secret-leak-history.test.sh`、`bash scripts/check-pii-leak.test.sh` | `scripts/check-secret-leak-history.test.sh`、`scripts/check-pii-leak.test.sh` | 残る（意図） |
...
- **残る（意図）**は証跡（D-003）と、他施設にも属しうる利用者本人（D-004）、送信済みメール（D-031）、**git 履歴に入った本番の識別子**（D-032）。最後のものだけ「意図して入れた」ものではなく、**入ってしまったものを消せないので残すと決める必要がある**（公開の判断と同時に）。
- **未確認**の 4 つ（D-021・D-022・D-030・D-041）は保持期間か中身をダッシュボードで見れば埋まる。
  Vercel / Supabase の確認は #757-35（設定ドリフト）の棚卸しと同時に行う。

## 限界

- **ダッシュボード側は未確認のまま。** Vercel と Supabase のログ保持期間、バックアップの中身、
  AI エージェントの観測ログは、この表では「未確認」と書いてあるだけで測っていない
  （#757-28 / #757-35）。**外部サービスの内部保持はこちらからは見えない。**
- **「消える」は削除経路を 1 つ確かめただけ。** 別の経路（Studio・service role・
  復元されたバックアップ）から同じデータが戻らないかは #757-23 の担当で、まだ測っていない。
- **列の粒度でしか見ていない。** 1 つの列に個人情報とそうでないものが混ざっている場合
  （自由入力の備考欄など）、この表は「残る」としか言えない。
- **新しい残存先が増えても自動では気づかない。** 表・ログ・外部送信を足したときに、
  ここへ行を足すのは人の作業。
- **履歴の走査は「秘密の形」しか見ない。** 鍵・トークンのパターンに当たらないもの
  （project ref・組織 ID・社内 URL・人の名前）は 0 件でも残っている。D-032 がその実例で、
  **走査が緑であることは「出したくないものが無い」ことを意味しない**。
- **別の clone・fork・バックアップに残った履歴はこちらから消せない。**
  仮に書き換えても、既に配られたものは戻らない。
