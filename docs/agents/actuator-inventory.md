...
| Setup（matcher `maintenance`） | `maintenance-digest.sh` | warning-only | `claude -p --maintenance` で fault-injection 訓練・hook 実走ドリル・docs 差分確認・依存の月次棚卸しの 4 期限を一括表示（issue #741、#757 の 21）。明示的に呼ばないと動かないため SessionStart の個別警告は残す |
