...
- ②feature/attemptのlabel規約化（issue #423ステップ3）は`.claude/workflows/aidd-phase2.js`
  ...
  埋め込み（既存プロンプト本文の中身は変更しないため`workflow-prompt-sync.test.js`には
  影響しない）、`scripts/log-subagent-hook-skeleton.sh`が`SubagentStop`ペイロードの
  `agent_transcript_path`が指すtranscriptファイルをテキストとしてgrepし、規約行を
