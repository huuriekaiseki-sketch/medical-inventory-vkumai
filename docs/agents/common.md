...
- fixtureは `scripts/eval-fixtures/<name>/` に `manifest.json`（agentType・プロンプトのビルド元モジュール・モデル・出力スキーマ）と `case-*/spec.md` + `case-*/expected.json` を置く形式。db-implには4ケース（①DB変更あり→pass ②「該当なし」明記→pass ③DB言及なし・ただし文脈から不要と推論可能→pass ④方式未確定でドメイン知識だけでは要否を推論不能→blocked）を用意済み。将来contract-writer等のプロンプトを追加する場合は `scripts/eval-fixtures/<name>/` を増やすだけでよい。
  - **旧・既知のカバレッジの穴（issue #401→#406で解消）**: 当初のcase-3は「DB言及なし→blocked」を意図していたが、実際にagentへ読ませたところ「クライアント側ソートのみで完結する機能なのでDB変更は不要」と合理的に推論してpass判定した（プロンプトの`blocked`条件は「DBという単語が無い」ではなく「要否を判断できない」ため、この推論はプロンプトの文言上は正当）。この結果、db-impl用3fixture全てが期待値passとなり`blocked`パスを検証するfixtureが1つも無い状態になっていた（issue #406）。永続化方式自体がSPEC上で未確定（案A/案Bのどちらか会議で保留）というcase-4を追加し、`blocked`パスの実カバレッジを確保した。
- 各fixtureはローカルの一時ディレクトリへリポジトリを `git clone --depth 1` してから実行する。本体の `supabase/migrations/` 等を実際に汚さないための隔離（fixture①はマイグレーションファイルを実際に書こうとするため）。
  - **既知の未検証事項（issue #401、issue #407で追跡中）**: Claude Codeセッション自身のBashツール経由で（＝既にサンドボックスされた親プロセスの子として）`npm run eval:workflows`を実行すると、fixture①の`claude -p`サブプロセスがWrite/Bashによるファイル書き込みを一律拒否され、正しい判断ができていても`fail`報告になる事例を観測した（2026-07-16の`case-4`追加時にも再現）。人間が通常のターミナルから実行した場合や本番のAIDD Workflow実行context（Workflow toolの中）で実行した場合に同じ問題が起きるかは未検証（親プロセスのサンドボックス制約が子の`claude -p`に継承される、セッション固有の現象である可能性がある）。
