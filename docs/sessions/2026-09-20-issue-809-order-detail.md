...
- 証拠状態: 実測 8 件（型検査・lint・unit 2420・build・統合 395・E2E 100・hook 155・テストの外からの直接攻撃 16 本）/ 未検証 0 件（ただし verify-claims は効いていない。04 欄）
...
- 作業中に見つけた別件: 1 件の発注の明細件数に上限が無い／ロット検索の画面側に `LOT_MAX_LENGTH = 100` の直書きが残っている／gap check が loop-observability の総行数で数えるので、フロー中に E2E が走ると判定が壊れる（実装フローの警告 actual=140・expected=21 のうち 116 行が E2E の記録だった）。起票済み: 順に #813・#814・#812
- 依存の変更: なし
...
| 直接攻撃の実測（テスト外） | ✅ 実施 (手動) | 攻撃表に無い入力 16 本を実サーバーへ当てた（使い捨ての spec。コミットしていない）。他施設の利用者で 10 本（素の ID・大文字の UUID・クエリで自施設を名乗る・select の上書き風・SQL 風の尾ひれ・or フィルタ風・末尾スラッシュ・発注 ID を返却の route へ・施設を名乗るヘッダ）→ 404（末尾スラッシュだけ 308 で素の ID へ寄る）。未ログイン 6 本（`x-middleware-subrequest` で proxy を飛ばす手口を含む）→ 307。応答に患者の情報なし。対照: 施設 A の本人は 200 |
...
- verify-claims は**効いていない**: Stop hook が自動で回す仕組みだが、このセッションでは 9 回呼ばれて 9 回とも fail_open で、1 回も検証していない。記録の開始（2026-07-15）から 2,475 回中 2,461 回が fail_open。主因は、検証器の応答がコードフェンスで包まれて `jq` が落ちること（同じ呼び方で 1 回再現して確定）。この PR の範囲外なので別 issue にした。**上の各証拠は verify-claims を通っていない**
