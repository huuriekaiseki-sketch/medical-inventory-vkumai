...
- 証拠状態: 実測 8 件（型検査・lint・unit 2420・build・統合 395・E2E 100・hook 155・テストの外からの直接攻撃 16 本）/ 未検証 0 件（verify-claims は未実施）
...
- 作業中に見つけた別件: 1 件の発注の明細件数に上限が無い／ロット検索の画面側に `LOT_MAX_LENGTH = 100` の直書きが残っている／gap check が loop-observability の総行数で数えるので、フロー中に E2E が走ると判定が壊れる（実装フローの警告 actual=140・expected=21 のうち 116 行が E2E の記録だった）。3 件とも起票キューに積んだ（issue 番号は作成後に PR へ追記する）
- 依存の変更: なし
