...
`claude -p` サブプロセス自体が失敗(タイムアウト・認証エラー・ネットワークエラー等)した場合は、内容判断の deny-by-default とは別扱いにする。**この場合はfail-open(警告のみでexit 0)とする**。
...
| 検証プロセス自体の失敗(タイムアウト等) | fail-open(exit 0、警告のみ) |
