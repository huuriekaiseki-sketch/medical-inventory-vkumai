...
**ルール:** `agent` の値域を2つに分離する。

- **人間の場合:** 固定文字列 `"human"` を使う（個人名は書かない。Gitのコミッターと違いログは長期蓄積されるため、実名を値として繰り返し書き込む設計にしない）
- **AIエージェントの場合:** `.claude/agents/` のagent定義名をそのまま使う（例: `implementer`, `reviewer`, `judge-panel`）

この2つの値域は文字列としても重複しない（agent定義名に`human`は存在しない）ため、後続の集計・要約スクリプトは `agent === "human"` かどうかだけで人間/AIを判定できる。
