...
#     2026-09-12 に `self` の 34 本を導入先 2 つで実測し、宣言と食い違うものは 0 本だったが、
#     これは**そのときの実測であって門ではない**（増えたときに自動では気づけない）
#   - 「対象なし」の**正しさは各検査の自己申告**。下の差分は「実態ありの木と空の木で
#     出力が変わるか」しか見ないので、検査が誤って「対象なし」と言えばそのまま通る。
#     2026-09-12 実測（3 つの木）: 出力が実行ごとに変わって必ず「見た」へ倒れる検査は **0 本**、
#     実態を見た結果が無言で空の木と区別できない検査も **0 本**——後者は
#     `check-plugin-check-coverage.test.sh` の scenario 5 が「対象の無い木では必ず
#     『対象なし』と言う」を強制した副産物で、空の木側が無言でなくなったため消えた
