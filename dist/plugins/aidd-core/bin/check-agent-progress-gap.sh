...
set +e
JUDGE_OUT="$(node --experimental-detect-module --no-warnings "$SCRIPT_DIR/../scripts/workflow-lib/agent-progress-gap.js" --actual "$ACTUAL_COUNT" --expected "$EXPECTED_COUNT")"
JUDGE_EXIT=$?
set -e
if [ -z "$JUDGE_OUT" ]; then
