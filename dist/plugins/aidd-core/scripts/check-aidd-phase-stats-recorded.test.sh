...
run_hook() {
  set +e
  # WHY(2026-09-12): stats の鍵は cwd のハッシュ（59〜60 行目）。本体は
  #      cd "${CLAUDE_PROJECT_DIR:-...}" するので、外から環境変数が入ると cwd が変わって
  #      鍵が食い違い、置いた記録を見つけられず「記録なし」と誤警告する。前提を固定する
  OUT="$(CLAUDE_PROJECT_DIR="$REPO_ROOT" \
    AIDD_PHASE_STATS_CHECK_SESSION_ID="$SESSION" \
