...
# DB マイグレーション配下等）を足して作る（issue #420 v1 セット B2）。設定が無ければ汎用分だけで判定する
...
# ドメイン語（このリポジトリ名にも含まれる語）が含まれているだけで常に誤検知する
# （実機で発見: リポジトリ名にドメイン語を含む /private/tmp/.../<repo>/... への無関係な書き込みで発火した）。
