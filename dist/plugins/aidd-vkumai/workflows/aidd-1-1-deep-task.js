...
while (shouldContinueSweepLoop(dryRounds, round, maxRounds, MIN_BUDGET_FOR_SWEEP_ROUND)) {
  ...
  const [uiResult, dataResult, dbResult, typesResult] = await parallel([
    () => agent(sweepPrompt, { label: `sweep-ui:R${round}`,    agentType: 'aidd-vkumai:sweep-ui',    phase: 'Sweep', schema: AGENT_RESULT_SCHEMA_PB, effort: 'low' }),
    () => agent(sweepPrompt, { label: `sweep-data:R${round}`,  agentType: 'aidd-vkumai:sweep-data',  phase: 'Sweep', schema: AGENT_RESULT_SCHEMA_PB, effort: 'low' }),
    () => agent(sweepPrompt, { label: `sweep-db:R${round}`,    agentType: 'aidd-vkumai:sweep-db',    phase: 'Sweep', schema: AGENT_RESULT_SCHEMA_PB, effort: 'low' }),
    () => agent(sweepPrompt, { label: `sweep-types:R${round}`, agentType: 'aidd-vkumai:sweep-types', phase: 'Sweep', schema: AGENT_RESULT_SCHEMA_PB, effort: 'low' }),
  ])
