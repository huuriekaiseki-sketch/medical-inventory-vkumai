...
- この Claude Code 用プラグイン v1 は Codex 設定を配布しない。Codex 向けの共通 hook は
  `aidd-codex` で別途配布する設計とし、`.codex/agents/*.toml` の配布可否は未確定。
...
- **実行系が無い環境では多くの hook が沈黙する。** 2026-09-11 に実測し直した数字は次のとおり
  ...
  **`jq` を数えておらず実態より狭かった**）:
  ...
  合わせた**実体 47 本**が母数。**この Claude Code 用プラグイン v1 では Codex 側は配布対象外**だが、同じスクリプトを呼ぶので
