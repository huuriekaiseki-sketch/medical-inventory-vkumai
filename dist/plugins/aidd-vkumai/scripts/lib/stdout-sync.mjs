...
//
// 限界:
//   - `console.error`（標準エラー）は替えていない。stderr は Node では常に同期なので
//     同じ問題は起きない（パイプでも同期）。
//   - **替えていないものは 1 か所で数える**。ここに件数を書くと必ずずれる——実際
//     2026-09-11 の最初の版は「残り 2 本」と書いたが、走査したら **4 本**あった
//     （`e2e/` が置換の走査の外にいた）。正本は `aidd.config.json` の
//     `stdoutSync.exemptions`（パスごとに**なぜ今も要るか**）で、
//     `scripts/check-stdout-truncation.test.sh` が「宣言の外に増えていないか」
//     「宣言が腐っていないか」を毎回見る。
//   - **「小さいから安全」は今の実装に対する判断**で、出力が増えれば崩れる。
//     だから免除の理由には「何を出しているか」を書かせている（増えたら読み直せるように）。
