...
//     同じ問題は起きない（パイプでも同期）。
//   - `.claude/workflows/lib/` の 2 本（`agent-progress-gap.js` / `loop-observability-gap.js`）は
//     替えていない。**理由は import の都合ではなく、出力が `{actualCount, expectedCount, hasGap}` の
//     1 行だけ（数十バイト）で 64KB に届きようがないから**。
//     出力が育つ設計に変えたら、そのときここへ足す。
//   - **「小さいから安全」は今の実装に対する判断**で、出力が増えれば崩れる。
//     判断の根拠（何を出しているか）を上に書いたのはそのため。
