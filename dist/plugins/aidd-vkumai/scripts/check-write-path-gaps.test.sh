...
# WHY(2026-09-12): 「違反なし（または対象なし）」と 1 行にまとめていたため、呼ぶ側
#      （aidd-check）が**見た結果の合格**と**対象が無いので黙った**を区別できなかった（C-025）。
#      どちらであるかを出力で確定させる。印の語「対象なし」は後者のときだけ出す。
if grep -qF -- "対象 0 件" <<<"$OUT"; then
  echo "  OK: 対象なし（この導入先は隙間の登録簿を持たない）"
elif grep -qF -- "violations=0" <<<"$OUT"; then
  echo "  OK: 違反なし（実態を見たうえで 0 件）"
else
