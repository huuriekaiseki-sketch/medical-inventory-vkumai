...
set +e
JUDGE_OUT="$(node --experimental-detect-module --no-warnings "$SCRIPT_DIR/../.claude/workflows/lib/loop-observability-gap.js" --actual "$ACTUAL_COUNT" --expected "$EXPECTED_COUNT")"
JUDGE_EXIT=$?
set -e
if [ -z "$JUDGE_OUT" ]; then
