...
埋め込んでいる欠陥: 層をまたぐ型不一致。`src/types/shift-handovers.ts` の `ShiftHandoverItem` は
`internalNote` を宣言していないが、`src/lib/shift-handovers/repository.ts` の `mapRow()` は
...
期待パスは 2 つ（`shift-handovers/repository.ts` と `types/shift-handovers.ts`）のいずれか。
