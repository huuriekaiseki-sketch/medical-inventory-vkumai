...
CHECKER="$SCRIPT_DIR/lib/check-roadmap-staleness.mjs"

# ロードマップの登録簿は導入先のもの（エンジンは共通側）。無ければ見るものが無い
if [ ! -f "$ROOT/scripts/lib/roadmap-registry.json" ]; then
  echo "=== scenario 0: この導入先にはロードマップの登録簿が無い ==="
  echo "  SKIP: scripts/lib/roadmap-registry.json が無いので対象なし"
  echo "ALL PASSED"
  exit 0
fi
...
# WHY(登録簿も置く、2026-09-09): 対象と対応表は登録簿（scripts/lib/roadmap-registry.json）から取る。
#      置かないと「飛ばした」で通ってしまい、**fixture が何も測らない**（実際にそうなった）
mkdir -p "$TMP/fx/scripts/lib"
cp "$ROOT/scripts/lib/roadmap-registry.json" "$TMP/fx/scripts/lib/"
...
import("./scripts/lib/check-roadmap-staleness.mjs").then(async (m) => {
  ...
  const out = []
  for (const [n, files] of Object.entries(m.loadRegistry(process.cwd())?.evidence ?? {})) {
