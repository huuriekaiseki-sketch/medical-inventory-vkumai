...
echo "=== scenario 12: diffハッシュが変わっても同一findingが残っていればretry_countは累積する(issue #351) ==="
rm -f "$MOCK_CALL_LOG"
echo "line12a" >> "$REPO/file.txt"
echo '{"findings": [{"severity": "critical", "description": "解消されない指摘", "evidence": "x:1"}]}' > "$MOCK_FINDINGS_FILE"
run_hook "s12"
assert_eq "$EXIT_CODE" "2" "1回目(新規diff・critical)はブロック"
RETRY_1="$(jq -r '.retry_count' "$STATE_DIR/s12.json")"
assert_eq "$RETRY_1" "1" "1回目のretry_countは1"
echo "line12b(無関係な追記)" >> "$REPO/file.txt"
run_hook "s12"
assert_eq "$EXIT_CODE" "2" "2回目(diffハッシュは変わったが同一finding)もブロック"
RETRY_2="$(jq -r '.retry_count' "$STATE_DIR/s12.json")"
assert_eq "$RETRY_2" "2" "同一findingが残っている場合はretry_countが2に累積する(diffハッシュを変えるだけで足踏みするズルを防ぐ)"

echo "=== scenario 13: diffハッシュが変わり指摘の中身も変わった場合はretry_countが1にリセットされる(issue #351) ==="
rm -f "$MOCK_CALL_LOG"
