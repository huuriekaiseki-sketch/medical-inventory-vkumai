...
echo "=== scenario 12: evidenceが実在しないファイルを指すcritical finding → minorへ格下げされブロックされない(issue #354) ==="
rm -f "$MOCK_CALL_LOG"
echo "line12" >> "$REPO/file.txt"
...
VERDICT_S12="$(jq -r '.last_verdict' "$STATE_DIR/s12.json")"
assert_eq "$VERDICT_S12" "pass" "状態ファイルはpassとして記録される(ブロックしていないため)"

echo "=== scenario 13: evidenceのファイルは実在するが行番号が範囲外のcritical finding → minorへ格下げ(issue #354) ==="
rm -f "$MOCK_CALL_LOG"
echo "line13" >> "$REPO/file.txt"
...
echo "=== scenario 14: evidenceが空/ファイルパスらしきトークンを含まないcritical finding → minorへ格下げ(issue #354) ==="
rm -f "$MOCK_CALL_LOG"
echo "line14" >> "$REPO/file.txt"
