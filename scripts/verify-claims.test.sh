...
echo "=== scenario 12: blockイベント(新規verifier呼び出し)が正しく記録される(issue #355) ==="
...
echo "=== scenario 13: block_retry(同一diff再ブロック)がverifier_called=falseで記録される(issue #355) ==="
...
echo "=== scenario 14: retry上限超過でretry_exhausted=trueが記録される(issue #355) ==="
RE="$(log_field "s5" "block" "retry_exhausted")"
assert_eq "$RE" "true" "上限超過後の最新blockエントリはretry_exhausted=true"

echo "=== scenario 15: 指摘解消後のpassイベントでwas_previously_blocked=trueが記録される(issue #355) ==="
rm -f "$MOCK_CALL_LOG"
echo "line15a" >> "$REPO/file.txt"
echo '{"findings": [{"severity": "critical", "description": "一時的な指摘", "evidence": "file.txt:1"}]}' > "$MOCK_FINDINGS_FILE"
run_hook "s15"
echo "line15b" >> "$REPO/file.txt"
echo '{"findings": []}' > "$MOCK_FINDINGS_FILE"
run_hook "s15"
assert_eq "$EXIT_CODE" "0" "指摘解消後はpass"
WAS_PREV="$(log_field "s15" "pass" "was_previously_blocked")"
assert_eq "$WAS_PREV" "true" "直前がblockedだったpassにはwas_previously_blocked=trueが記録される(=修正が効いたシグナル)"

echo "=== scenario 16: 通常passではwas_previously_blocked=falseが記録される(issue #355) ==="
...
echo "=== scenario 17: fail-open(検証プロセス失敗)がfail_open_reason=verifier_errorで記録される(issue #355) ==="
FOR="$(log_field "s7" "fail_open" "fail_open_reason")"
assert_eq "$FOR" "verifier_error" "検証プロセス失敗はfail_open_reason=verifier_errorで記録される"

echo "=== scenario 18: fail-open(同時実行数上限)がfail_open_reason=circuit_breakerで記録される(issue #355) ==="
...
echo "=== scenario 19: fail-open(出力解析失敗)がfail_open_reason=parse_errorで記録される(issue #355) ==="
rm -f "$MOCK_CALL_LOG"
echo "line19" >> "$REPO/file.txt"
echo 'not-json-output' > "$MOCK_FINDINGS_FILE"
run_hook "s19"
assert_eq "$EXIT_CODE" "0" "出力解析失敗はfail-openでexit 0"
FOR="$(log_field "s19" "fail_open" "fail_open_reason")"
assert_eq "$FOR" "parse_error" "出力解析失敗はfail_open_reason=parse_errorで記録される"
echo '{"findings": []}' > "$MOCK_FINDINGS_FILE"

echo "=== scenario 20: .skipマーカー使用がskip_usedイベントとして記録される(issue #355) ==="
