...
echo "=== scenario 11: evidenceが実在しないファイルを指すcritical finding → minorへ格下げされブロックされない(issue #354) ==="
rm -f "$MOCK_CALL_LOG"
...
VERDICT_S11="$(jq -r '.last_verdict' "$STATE_DIR/s11.json")"
assert_eq "$VERDICT_S11" "pass" "状態ファイルはpassとして記録される(ブロックしていないため)"

echo "=== scenario 12: evidenceのファイルは実在するが行番号が範囲外のcritical finding → minorへ格下げ(issue #354) ==="
rm -f "$MOCK_CALL_LOG"
...
echo "=== scenario 13: evidenceが空/ファイルパスらしきトークンを含まないcritical finding → minorへ格下げ(issue #354) ==="
rm -f "$MOCK_CALL_LOG"
