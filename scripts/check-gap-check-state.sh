...
# 配ったとき cwd は導入先で scripts/ が無く、cwd 相対だと常に見つからない。リポジトリ内では同じ場所を指す
GAP_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LOOP_GAP_CMD="${GAP_CHECK_LOOP_CMD:-$GAP_SCRIPT_DIR/check-loop-observability-gap.sh}"
PROGRESS_GAP_CMD="${GAP_CHECK_PROGRESS_CMD:-$GAP_SCRIPT_DIR/check-agent-progress-gap.sh}"
