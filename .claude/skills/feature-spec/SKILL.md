...
- **UI変更（見た目のレイアウトだけでなく、処理中表示・モーダル開閉・バリデーション表示タイミング等のJS/UXの挙動変化も含む）が絡む機能では、Part 1を書き始める前に必ず`design`スキルでClaude Designのモックを作り、その結果を見ながらPart 1を書く**（文章だけを先に書いて後からモックを足す、という順序にしない。文章のみでの代替も不可）。
  DB/API/バックエンドロジックのみで画面にも操作感にも変化が無い機能は対象外（従来通り文章のみでよい）。
  - **既存画面を変更する場合は、Before(現状)とAfter(変更後)を両方作り、並べて比較できるようにする**（後述のBefore/Afterパターン）。新規に画面そのものを作る場合はBeforeが存在しないためAfterのみでよい。
  - 状態が変わるインタラクションを含む場合は、静的な絵ではなく**実際にクリックして動くモック**（`is_interactive: true`）にする。作って終わりにせず、`mock-capture.mjs`等で実際に動かして検証してから次に進む（見た目だけ作って動作未確認のまま仕様書に貼らない）。
