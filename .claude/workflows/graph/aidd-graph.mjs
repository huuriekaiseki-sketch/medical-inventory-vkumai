...
export const WORKFLOWS = {
  ...
  'aidd-1-1-deep-task': {
    ...
    nodes: [
      ...
      // WHY(agentType、issue #791): 2026-09-18 まで Draft Spec 以降の役割には agentType が無く、
      //      「既定のワークフロー用サブエージェント」＝全ツール持ちで起動していた。
      //      その結果、採点役が製品コードを編集して git commit まで実行した（E-094）。
      { id: 'tree-before', phase: 'Sweep', kind: 'agent', label: 'capture-tree:before（HEAD と作業ツリーを控える）', agentType: 'reviewer', model: 'haiku', effort: 'low' },
      { id: 'draft', phase: 'Draft Spec', kind: 'agent', label: 'draft-spec', agentType: 'spec-drafter', model: 'sonnet', effort: 'medium' },
      { id: 'draft-gate', phase: 'Draft Spec', kind: 'gate', label: 'draftSpec.status === pass（deny-by-default）', veto: true },
      { id: 'find', phase: 'Find', kind: 'agent', label: 'find:<lens>（logic/data/security/ux/performance）', agentType: 'reviewer', model: 'haiku', effort: 'low', fanout: 5, parallelGroup: 'find' },
      { id: 'verify', phase: 'Adversarial Verify', kind: 'agent', label: 'verify:<i>（critical/important の指摘ごと。minor は自動生存）', agentType: 'adversarial-verify', model: 'opus', effort: 'medium', fanout: 'findings', parallelGroup: 'verify' },
      { id: 'critic2', phase: 'Completeness Critic', kind: 'agent', label: 'completeness-critic-2', agentType: 'completeness-critic', model: 'sonnet', effort: 'medium' },
      { id: 'propose', phase: 'Judge Panel', kind: 'agent', label: 'propose:<stance>（MVP優先/リスク最小/拡張性重視）', agentType: 'proposer', model: 'sonnet', effort: 'medium', fanout: 3, parallelGroup: 'propose' },
      { id: 'divergence', phase: 'Judge Panel', kind: 'code', label: 'computeDivergence（bigram Jaccard ≥ 0.5 で分岐判定。正本 lib/judge-panel.js）' },
      { id: 'score', phase: 'Judge Panel', kind: 'agent', label: 'score:<案>:<lens>（correctness/security/ux。分岐ありのときのみ）', agentType: 'judge-panel', model: 'haiku', effort: 'low', fanout: 9, parallelGroup: 'score' },
      { id: 'synthesize', phase: 'Synthesize', kind: 'agent', label: 'synthesize', agentType: 'judge-panel', model: 'sonnet', effort: 'high' },
      { id: 'tree-after', phase: 'Synthesize', kind: 'agent', label: 'capture-tree:after（開始時と突き合わせる）', agentType: 'reviewer', model: 'haiku', effort: 'low' },
      { id: 'tree-guard', phase: 'Synthesize', kind: 'gate', label: 'HEAD と作業ツリーが開始時のまま（停止①より前に木を変えていないか）', veto: true },
    ...
    edges: [
      ...
      { from: 'tree-before', to: 'sweep-ui', on: 'always', note: '開始時の HEAD と作業ツリーを控えてから Sweep に入る（issue #791）' },
      ...
      { from: 'synthesize', to: 'tree-after', on: 'always' },
      { from: 'tree-after', to: 'tree-guard', on: 'always' },
      { from: 'tree-guard', to: 'end', on: 'pass', note: 'HEAD も作業ツリーも開始時のまま' },
      { from: 'tree-guard', to: 'end', on: 'blocked', blockedAt: 'Tree Guard', returnsTo: 'human', note: '停止①より前なのに木が変わっている。git log / git status で何が入ったかを確かめ、意図しない変更なら巻き戻してから人に報告する（issue #791）' },
    ],
  },
  ...
  'aidd-phase2': {
    ...
    nodes: [
      ...
      { id: 'gate-retry', phase: 'Review', kind: 'gate', label: 'shouldBlock（修正そのものの成否。ここでコードが変わり統合結果は失効する）', veto: true },
      { id: 'integration-recheck', phase: 'Review', kind: 'agent', label: 'integration-recheck:R<n>（修正後の木で npm test / lint / tsc と specHash を取り直す）', agentType: 'integrator' },
      { id: 'gate-recheck', phase: 'Review', kind: 'gate', label: 'shouldBlock（取り直した統合ゲート。pass で integrationFresh が戻る）', veto: true },
      { id: 'end', phase: 'Review', kind: 'end', label: 'DONE（computeDone: 全 impl pass ∧ integrate pass ∧ 全 review pass ∧ specHash 一致 ∧ その統合結果が最後の変更より後）→ 停止②' },
    ],
    edges: [
      ...
      { from: 'implementer-retry', to: 'gate-retry', on: 'always' },
      { from: 'gate-retry', to: 'end', on: 'blocked', blockedAt: 'Review Retry', returnsTo: 'stop-2', note: '修正そのものが fail/blocked（直せない・対象が特定できない）。従来は status を見ずに次ラウンドへ回していた' },
      { from: 'gate-retry', to: 'integration-recheck', on: 'pass' },
      { from: 'integration-recheck', to: 'gate-recheck', on: 'always' },
      { from: 'gate-recheck', to: 'end', on: 'blocked', blockedAt: 'Integrate Recheck', returnsTo: 'human', note: '修正で test/lint/tsc が赤になった、または修正中に SPEC.md が変わり specHash が不一致' },
      { from: 'gate-recheck', to: 'review', on: 'loop', note: '取り直した統合ゲートが緑（integrationFresh = true）になってから次のレビューへ' },
