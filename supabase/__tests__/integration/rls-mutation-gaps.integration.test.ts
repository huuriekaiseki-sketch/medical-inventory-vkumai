...
//
//        RM-002 施設スコープの書き込みから aal2 を外しても気づかない
//              → aal2 のテストは **RPC 経由**しか見ておらず、表への直接書き込みを見ていなかった
//        RM-003 仕入価格の書き込みを閲覧者にも許しても気づかない
//              → 閲覧者のテストは消耗品と発注 RPC だけで、仕入価格を見ていなかった
//        RM-006 施設の作成から admin 判定を外しても気づかない
//              → マスタ境界のテストは商品・カテゴリ・代理店商品だけで、施設を見ていなかった
//        RM-010 回数のカウンタを認証済み利用者に開いても気づかない
...
describe('表への直接書き込みにも aal2 が要る [P-030]', () => {
  // WHY(RM-002): aal2 のテストは RPC 経由しか見ていなかった。RPC には関数内の判定もあるので、
...
describe('閲覧者は仕入価格を書けない [P-020]', () => {
  // WHY(RM-003): 閲覧者のテストは消耗品と発注 RPC だけを見ており、仕入価格を見ていなかった。
...
describe('施設の作成は admin だけ [P-021]', () => {
  // WHY(RM-006): マスタ境界のテストは商品・カテゴリ・代理店商品を見ていたが、
...
describe('回数のカウンタはログイン済み利用者からも見えない [P-064]', () => {
  // WHY(RM-010): カウンタのテストは anon だけを見ていた。**ログイン済みの利用者**に開いても
