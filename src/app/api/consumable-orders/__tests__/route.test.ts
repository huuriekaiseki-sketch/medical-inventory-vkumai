...
describe('GET /api/consumable-orders', () => {
  it('未認証の場合は401を返す', async () => {
    unauthenticated()
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1'))
  ...
  it('facility_id未指定・非adminの場合は400を返す', async () => {
    ...
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders'))
  ...
  it('未所属施設を指定した場合は403を返す', async () => {
    ...
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f9'))
  ...
  it('limit/offset省略時はデフォルト値(50, 0)を使う', async () => {
    ...
    expect(mockListConsumableOrders).toHaveBeenCalledWith(expect.anything(), 'f1', 50, 0)
  })

  it('limitが数値でない場合は400を返す', async () => {
    authenticated()
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1&limit=abc'))
    expect(res.status).toBe(400)
    expect(mockListConsumableOrders).not.toHaveBeenCalled()
  })
  ...
  it('offsetが数値でない場合は400を返す', async () => {
    authenticated()
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1&offset=xyz'))
    expect(res.status).toBe(400)
    expect(mockListConsumableOrders).not.toHaveBeenCalled()
  })
  ...
  it('limitが0の場合は400を返す（1未満は不可）', async () => {
    authenticated()
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1&limit=0'))
    expect(res.status).toBe(400)
    expect(mockListConsumableOrders).not.toHaveBeenCalled()
  })
  ...
  it('limitが負数の場合は400を返す', async () => {
    authenticated()
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1&limit=-1'))
    expect(res.status).toBe(400)
    expect(mockListConsumableOrders).not.toHaveBeenCalled()
  })
  ...
  it('limitが上限(200)を超える場合は400を返す', async () => {
    authenticated()
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1&limit=9999'))
    expect(res.status).toBe(400)
    expect(mockListConsumableOrders).not.toHaveBeenCalled()
  })
  ...
  it('limitが上限ちょうど(200)の場合は200を返す', async () => {
    authenticated()
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1&limit=200'))
    expect(res.status).toBe(200)
    expect(mockListConsumableOrders).toHaveBeenCalledWith(expect.anything(), 'f1', 200, 0)
  })

  it('offsetが負数の場合は400を返す', async () => {
    authenticated()
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1&offset=-5'))
    expect(res.status).toBe(400)
    expect(mockListConsumableOrders).not.toHaveBeenCalled()
  })
  ...
  it('正常系: limit・offset指定時にそのまま渡る', async () => {
    authenticated()
    mockListConsumableOrders.mockResolvedValue([{ id: 'c1' }])
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1&limit=10&offset=20'))
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.orders).toEqual([{ id: 'c1' }])
    expect(mockListConsumableOrders).toHaveBeenCalledWith(expect.anything(), 'f1', 10, 20)
  })

  it('例外発生時は500を返す', async () => {
    authenticated()
    mockListConsumableOrders.mockRejectedValue(new Error('DB error'))
    const res = await GET(new NextRequest('http://localhost/api/consumable-orders?facility_id=f1'))
...
describe('GET /api/consumable-orders: admin × facility_id 省略', () => {
