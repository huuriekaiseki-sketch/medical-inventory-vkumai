...
- `/goal` の条件は**ターン数の上限を自然文の条件文に埋め込む形**で指定する
  （例：「全フェーズ(Phase 1〜5)完了、または15ターン経過で停止」。`/goal`は時間ベースの上限を
  直接サポートしていない（issue #441で実機確認済み）ため、時間で区切りたい場合もターン数に
  換算して埋め込むこと。この例示値はFable 5の実測データ（旧世代比で1ターンあたり平均出力
  トークン数が約1.3倍）を踏まえ20→15に再校正した（issue #498。根拠・データ期間・限界は
  `docs/agents/decisions.md`参照）ため、モデル世代が変わった場合は同様に見直すこと）
